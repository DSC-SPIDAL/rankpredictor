...
Usage:
    rplog.py --extract --input inputfile --output outputfile
    example:
...
CMDMAP_HEXCOL={'C':['rank','completed_laps'],
               'O':['resultid','rank','overall_rank','start_position','best_lap'],
               'L':[],
               'S':['last_lap']
            }
...
class rplog:
    ...
    def __init__(self, logfile, outfile='', deli=chr(0xa6)):
        ...
        self.cmd_outf = {}
        for cmd in CMDMAP:
            self.cmd_dict[cmd] = _buliddict(CMDMAP[cmd])
            self.cmd_outf[cmd] = open(cmd + '_' + outfile + '.csv', 'w')
            self.cmd_outf[cmd].write(','.join(CMDMAP[cmd]) + '\n')
    ...
    def extract(self, save_telemetry = False):
        ...
        for line in self.inf:
            ...
            elif cmd[1] in CMDMAP:
                if raceStartFlag:
                    # convert time
                    for key in self.cmd_dict[cmd[1]]:
                        #if key.find('time') != -1 or key in CMDMAP_HEXCOL[cmd[1]]:
                        # exception 'time_line'
                        id = self.cmd_dict[cmd[1]][key] + RECSTARTCOL
                        if key.find('time') > 0:
                            items[id] = str(_hex2int(items[id])*1.0/10000)
                        elif key in CMDMAP_HEXCOL[cmd[1]]:
                            items[id] = str(_hex2int(items[id]))


                    #save completed laps info
                    self.cmd_outf[cmd[1]].write(','.join(items[RECSTARTCOL:]) + '\n')
